...
The resource specification is comprised of resource definitions and a resource read handler.
The resource definition includes `name`, `description`, and `MIME type`.
...
The second argument is an `McpSchema.ReadResourceRequest`.
...
When a compatible client connects to a stateful server, the server can request additional information from the user:
